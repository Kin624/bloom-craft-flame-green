--[[
  *** CPM2 AUTO-UPDATED FROM LATEST DUMP (dump_offset_updater.py) ***
  Home menu cleaned: removed One-Click Everything / Full God Account
  from the main list. All working feature functions remain below.
  Offsets auto-mapped Class::Method from 1.3.2.3 dump → latest dump.cs

  Update offsets only in the OFFSET TABLE header when game updates.
--]]
...
--[[ ══════════════════════════════════════════════════════════
  ...
  MONEY / PLAYERPREFS
    0x67521D0  UnityEngine.PlayerPrefs.GetFloat(string,float)
               Instant Money: rewrite body to return ~50M float bits
    0x6752138  PlayerPrefs.SetFloat(string,float)          (related)
    0x675225C  PlayerPrefs.SetString(string,string)
               ID Changer patches call site at +0x9C (MOV W0,WZR style)

  CURRENCY (CurrencyManagerController) — optional hooks
    0x31DC7BC  SetMoneyPrefs(AdvancedInt)
    0x31DC858  SetMoney(AdvancedInt)
    0x31DCAEC  SetCoins(AdvancedInt, Source)
    0x31DCAF4  AddMoney(AdvancedInt)
    0x31DCD88  IsEnoughCoins(int,bool)  → force true = free purchases

  RACE / LAP  (class LapRaceCar + CircuitRaceControlller)
    0x30035F0  LapRaceCar.IsFinalLap()              → always true (1 lap win)
    0x3002E5C  LapRaceCar.IsCheatFinish()           → false (bypass reward flag)
    0x30031AC  LapRaceCar.OnLapFinish()             → finish hook
    0x3003424  LapRaceCar.GetBestLap()              → freeze lap time helpers
    0x30033DC / 0x300342C / 0x30034C0 / 0x2FFA8A4 / 0x2FFA878
    0x30037C8 / 0x3A7A8B0 / 0x3A7A884 / 0x3A8301C   more time getters
    0x3003168 … 0x3BB6500  GetPenaltyDistance family → zero penalties
    0x3003088 … 0x3A829C4  GetTotalProgress family  → max progress
    0x3003998  LapRaceCar.CalculatePos()            → teleport / auto pos
    0x3165F18  CircuitRaceControlller.CheckParent() → local bypass
    0x3158ADC  CircuitCarStearing.ChangeSteeringState → dumb AI enemies
    0x319D114  LemanRaceController.SetTime(int)     → Le Mans timer
    0x3A826A8  IsCheatFinish (server path)          → bypass server

  RALLY  (class RallyCar / RallyController)
    0x35545B4  RallyCar.IsCarOffRoad(out float)     → never off-road
    0x3553538  RallyCar.get_Penalty()               → 0 penalty
    0x3553540  RallyCar.get_MissedCheckpoints()     → 0 missed
    0x3553B8C  RallyCar.AddPenalty(float,type)      → RET (no add)
    0x3555898  ApplyMissedCheckpointsPenalty        → RET

  GEARBOX / ENGINE / TUNING
    0x3884C1C  CheckGearboxNotCompatible(PartData,out bool)
               → force compatible (unlock all gearbox)
    0x3603ED4  CheckNotCompatibleEngine(PartItem,int)
               → force engine part OK
    0x30A19DC  EncryptEngine(int)                   → identity / bypass
    0x30A19EC  (alt engine gate, same area)
    0x360E718  get_CurrentUnixTimeSeconds()         → service-time bypass
    0x3854158  CheckerEngineCheating(out int2)      → disable detect
    0x3605C3C  (engine alt gate)
    0x358375   (gearbox alt / short RVA variant in older scripts)

  UNLOCK / SHOP
    0x37E3B7C  Police inventory IsBought(CarInfo)   → always bought
    0x30C5730  hasHouse(int)                        → house / police house
    0x3673D24 / 0x367387C / 0x3673C2C
               CheckPaintBought / paint gates       → unlock paints
    0x372A524  GetCoinPriceForKit(BodyKitType,int)  → free bodykits
    0x361DBE8  get_BodyKitsMoneyPrice()             → 0 price
    0x32F6FE4  GetClassVehicle(int,int)             → class editor (F1/GR6)
    0x39AE43C  get_TyreHealth()                     → tyre 0%/100% hooks

  LOGO / RANK
    0x3069CC0  GetOwnRank()                         → fake rank logo
               MOV W0,#rank ; RET   (King/YT/TikTok/IG/Dev)

  ACHIEVEMENTS / MISSIONS
    0x32774E8  GetDistance() (mission path)         → taxi/delivery/cargo
  ...
  SLOTS
    0x367D090+0xC0  inventory slot limit gate


  EXTRA BYPASSES (dump 1.3.2.3)
    0x31DCE40  IsEnoughMoney(int,bool)           → true
    0x31DCEF8  IsMoneyEnough(int)                → true
    0x3679A48  Parts IsBought(body,type,item)    → true
    0x3726688  AirSus IsBought(CarInfo)          → true
    0x337395C  IsBannedCar                       → false
    0x3102558  CheckerNFSCars                    → RET
    0x33739E4  CheckTradeCarNotAllowed(int)      → 0
    0x33739E4  CheckTradeCarNotAllowed(string)   → 0
    0x35D4640  IsVehicleAllowed(event,class)     → true
    0x3002358  AddPenaltyDistance                → RET
    0x3002368  AddPenaltyTime                    → RET
    0x3945444  get_CheckHashLimitExceeded        → false
    0x3945454  get_CorrectHash                   → true*
    0x3649BB4  IsBlackVehicle                    → false
    0x309EE0C  UnlockCar(int,int)                (call-site / room)
...
local supportedVersions = {"1.3.2.3", "1.3.3.0", "1.3.3.1", "1.4.0.0"}
...
local MONEY_OFF = 0x67521D0  -- PlayerPrefs.GetFloat
...
-- RVA: 0x31DCD88  CurrencyManagerController.IsEnoughCoins
function freePurchases()
    -- CurrencyManagerController.IsEnoughCoins @ 0x31DCD88
    local ranges = gg.getRangesList("libil2cpp.so")
...
local UNLOCK_ALL_OFFSETS = {
    0x3673C2C, 0x367387C, 0x3673D24, 0x3398A58,
    0x3124924, 0x3679A48, 0x367ADC4
}
...
local function applyPolice()
    gg.setValues({
        {address=LibStart+0x37E3B7C,   flags=gg.TYPE_DWORD, value=-763363296},
        {address=LibStart+0x37E3B7C+4, flags=gg.TYPE_DWORD, value=-698416192},
    })
    gg.toast("✅ Police ON")
end
...
local function revertPolice()
    gg.setValues({
        {address=LibStart+0x37E3B7C,   flags=gg.TYPE_DWORD, value=-132182018},
        {address=LibStart+0x37E3B7C+4, flags=gg.TYPE_DWORD, value=-1275068159},
    })
    gg.toast("🔵 Police OFF")
end
...
local function applyAirSus()
    pcall(function()
        gg.setValues({
            {address=LibStart+0x37266B0+0x448, flags=gg.TYPE_DWORD, value=-721215457},
            {address=LibStart+0x37C6634-0x28C, flags=gg.TYPE_DWORD, value=335544398},
        })
    end)
...
local function revertAirSus()
    pcall(function()
        gg.setValues({
            {address=LibStart+0x37266B0+0x448, flags=gg.TYPE_DWORD, value=905972544},
            {address=LibStart+0x37C6634-0x28C, flags=gg.TYPE_DWORD, value=1795687071},
        })
    end)
    gg.toast("🔵 Air Suspension OFF")
end

-- Tyres (offset 0x39AE43C)
local TYRES_OFF = 0x39AE43C  -- get_TyreHealth
...
function unlockallgb()   -- CheckGearboxNotCompatible @ 0x3884C1C
    patchPair(0x3884C1C)
    gg.toast("✅ Unlock All Gearbox")
end

function unlcokpartenmgine()  -- CheckNotCompatibleEngine @ 0x3603ED4
    patchPair(0x3603ED4)
...
function bypasseservvicetrime()  -- get_CurrentUnixTimeSeconds @ 0x360E718
    libs("libil2cpp.so")
    gg.setValues({
        {address=lib+0x360E718,   flags=gg.TYPE_DWORD, value="h200080D2"},
        {address=lib+0x360E718+4, flags=gg.TYPE_DWORD, value="hC0035FD6"},
    })
    gg.toast("✅ Bypass Service Time")
end

function buyefasf()   -- ApplySuspensionValueAsync area @ 0x3854FC4
    patchPair(0x3854FC4)
    gg.toast("✅ Bypass Detect Engine & GB")
end

function bypassgdetece()  -- CheckerEngineCheating @ 0x3854158 (+ 0xCB0 secondary)
    libs("libil2cpp.so")
    gg.setValues({
        {address=lib+0x3854158,       flags=4, value="h200080D2"},
        {address=lib+0x3854158+0xCB0, flags=4, value="hC0035FD6"},
    })
...
function bypassgtime()   -- alias / bypass GB Time
    libs("libil2cpp.so")
    gg.setValues({
        {address=lib+0x360E718+4, flags=4, value="h200080D2"},
        {address=lib+0x360E718,   flags=4, value="hC0035FD6"},
    })
...
function bypassengine1()   -- 0x3605C3C
    libs("libil2cpp.so")
    gg.setValues({
        {address=lib+0x3605C3C,   flags=4, value="h200080D2"},
        {address=lib+0x3605C3C+4, flags=4, value="hC0035FD6"},
    })
...
local CheatDetect = 0x3002E5C  -- LapRaceCar.IsCheatFinish
local Time1  = 0x3003424  -- LapRaceCar.GetBestLap;  local Time2  = 0x30033DC
local Time3  = 0x300342C;  local Time4  = 0x30034C0
local Time6  = 0x2FFA8A4;  local Time7  = 0x2FFA878
local Time8  = 0x30037C8;  local Time9  = 0x3A7A8B0
local Time10 = 0x3A7A8B0;  local Time11 = 0x3A7A884
local Time12 = 0x3A8301C
local Penalty1  = 0x3003168;  local Penalty2  = 0x3003508
local Penalty3  = 0x2FFA898;  local Penalty4  = 0x3A79F7C
...
local Distance1 = 0x3003088;  local Distance2 = 0x3003178
local Distance3 = 0x3003124;  local Distance4 = 0x2FFF9A8
local Distance5 = 0x3A82970;  local Distance6 = 0x3A829C4
local BypassLocal   = 0x3165F18  -- CircuitRaceControlller.CheckParent
local TeleportPatch = 0x3003998  -- LapRaceCar.CalculatePos
local LemansHook    = 0x319D114  -- LemanRaceController.SetTime
local Finish        = 0x30031AC  -- LapRaceCar.OnLapFinish
...
function racing1()  -- LapRaceCar.IsFinalLap @ 0x30035F0 — force 1-lap finish
    gg.setVisible(false)
    local ranges = gg.getRangesList("libil2cpp.so")
    if not ranges or #ranges < 2 then gg.toast("nothing found") return end
    local libil2cpp = ranges[2].start
    gg.setValues({
        {address=libil2cpp+0x30035F0,   flags=gg.TYPE_DWORD, value=-763363296, freeze=true},
        {address=libil2cpp+0x30035F0+4, flags=gg.TYPE_DWORD, value=-698416192, freeze=true},
    })
    gg.toast('✅ Laps Hooked — 1 Lap')
end

function racing2()  -- CircuitCarStearing.ChangeSteeringState @ 0x3158ADC — dumb AI
    gg.setVisible(false)
    local ranges = gg.getRangesList("libil2cpp.so")
    if not ranges or #ranges < 2 then gg.toast("nothing found") return end
    local libil2cpp = ranges[2].start
    gg.setValues({
        {address=libil2cpp+0x3158ADC,   flags=gg.TYPE_DWORD, value=-763363328, freeze=true},
        {address=libil2cpp+0x3158ADC+4, flags=gg.TYPE_DWORD, value=-698416192, freeze=true},
    })
...
function rally2()  -- Hook All Penaltys (bypass off-road / missed checkpoint penalties)
    ...
    gg.setValues({
        ...
        {address=libil2cpp+0x35545B4,   flags=gg.TYPE_DWORD, value=-763363328, freeze=true},
        {address=libil2cpp+0x35545B4+4, flags=gg.TYPE_DWORD, value=-698416192, freeze=true},
        -- get_Penalty~RallyCar
        {address=libil2cpp+0x3553538,   flags=gg.TYPE_DWORD, value=-763363328, freeze=true},
        {address=libil2cpp+0x3553538+4, flags=gg.TYPE_DWORD, value=-698416192, freeze=true},
        -- get_MissedCheckpoints~RallyCar
        {address=libil2cpp+0x3553540,   flags=gg.TYPE_DWORD, value=-763363328, freeze=true},
        {address=libil2cpp+0x3553540+4, flags=gg.TYPE_DWORD, value=-698416192, freeze=true},
        -- AddPenalty~RallyCar
        {address=libil2cpp+0x3553B8C,   flags=4, value="-698416192"},
        -- ApplyMissedCheckpointsPenalty~RallyCar
        {address=libil2cpp+0x3555898,   flags=4, value="-698416192"},
    })
...
function bypassserver()  -- IsBlackVehicle — Bypass black car server check
    libs("libil2cpp.so")
    gg.setValues({
        {address=lib+0x3649BB4,   flags=4, value="h200080D2"},
        {address=lib+0x3649BB4+4, flags=4, value="hC0035FD6"},
    })
...
local ClassEditor = 0x32F6FE4  -- GetClassVehicle(performanceScore, carId)
...
function IDChanger()
    local target   = LibStart + 0x675225C + 0x9C
...
local function taxiDeliveryCargo()
    libs("libil2cpp.so")
    gg.setValues({
        {address=lib+0x32774E8,   flags=gg.TYPE_DWORD, value="h528BF520"},
        {address=lib+0x32774E8+4, flags=gg.TYPE_DWORD, value="h72AB0C60"},
        {address=lib+0x32774F0,   flags=gg.TYPE_DWORD, value="h1E270000"},
        {address=lib+0x32774F0+4, flags=gg.TYPE_DWORD, value="hD65F03C0"},
    })
...
DRAG[1]={}
DRAG[2]={}
DRAG[1].address=GG+0x3069CC0  -- GetOwnRank()
DRAG[1].value='hC00080D2'
DRAG[1].flags=4
DRAG[2].address=GG+(0x3069CC0+0x4)
...
function ThirtyTwoSlots()
    ...
    local target = base + 0x367D090 + 0xC0
...
function policeautoset()
   --[[      	
		// RVA: 0x37E3B7C Offset: 0x37E3B7C VA: 0x37E3B7C Slot: 10
	public override bool IsBought(CarInfo carInfo) { }]]
    local LibStart = gg.getRangesList('libil2cpp.so')[2].start

    local KINZI = {
        {address = LibStart + 0x37E3B7C, value = "~A8 MOV  X0, #0x1", flags = gg.TYPE_DWORD},
        {address = LibStart + 0x37E3B7C + 4, value = "~A8 RET", flags = gg.TYPE_DWORD, },
    }
...
function unlockPolice()
    ...
    local patch = {
        {address = base + 0x37E3B7C, value = 'D2800020h', flags = 4},
        {address = base + 0x37E3B7C + 4, value = 'D65F03C0h', flags = 4}
    }
    gg.setValues(patch)
...
function hasHouse() --- Police Unlock
   --[[	// RVA: 0x30C5730 Offset: 0x30C5730 VA: 0x30C5730
...
KINZI={} --- IsBought
KINZI[1]={}
KINZI[1].address=GG+0x30C5730+0
KINZI[1].value='h200080D2'
KINZI[1].flags=4
KINZI[2]={}
KINZI[2].address=GG+0x30C5730+4
...
gg.sleep(100)
    ...
    local base = gg.getRangesList('libil2cpp.so')[2].start
    local DRAG = {
        {address = base + 0x3673D24, value = '~A8 MOV  X0, #0x1', flags = 4},
        {address = base + 0x3673D24 + 4, value = '~A8 RET', flags = 4}
    }
    gg.setValues(DRAG)
 
    local DRAG = {
        {address = base + 0x367387C, value = '~A8 MOV  X0, #0x1', flags = 4},
        {address = base + 0x367387C + 4, value = '~A8 RET', flags = 4}
    }
    gg.setValues(DRAG)
        local DRAG = {
        {address = base + 0x3673C2C, value = '~A8 MOV  X0, #0x1', flags = 4},
        {address = base + 0x3673C2C + 4, value = '~A8 RET', flags = 4}
    }
    gg.setValues(DRAG)
         local KINZI = {
        {address = LibStart + 0x3673D24, value = "-2999674700105252832", flags = gg.TYPE_QWORD},
    }
    gg.setValues(KINZI)
      local KINZI = {
        {address = LibStart + 0x367387C, value = "-2999674700105252832", flags = gg.TYPE_QWORD},
    }
    gg.setValues(KINZI)
    local KINZI = {
        {address = LibStart + 0x3673C2C, value = "-2999674700105252832", flags = gg.TYPE_QWORD},
    }
    gg.setValues(KINZI)
    gg.toast(" unlocked")
    gg.toast("༒ON༒")
...
DRAG[1]={}
DRAG[2]={}
DRAG[1].address=GG+0x372A524
DRAG[1].value='~A8 MOV  X0, #0x1'
DRAG[1].flags=4
DRAG[2].address=GG+(0x372A524+0x4)
DRAG[2].value='~A8 RET'
DRAG[2].flags=4
gg.setValues(DRAG) 
GG=gg.getRangesList('libil2cpp.so')[2].start
...
DRAG[1]={}
DRAG[2]={}
DRAG[1].address=GG+0x361DBE8
DRAG[1].value='h000080D2'
DRAG[1].flags=4
DRAG[2].address=GG+(0x361DBE8+0x4)
DRAG[2].value='hC0035FD6'
DRAG[2].flags=4
gg.setValues(DRAG)
    ...
    local KINZI = {
        {address = LibStart + 0x361DBE8, value = "-2999674700105252832", flags = gg.TYPE_QWORD},
...
if proceed == nil or proceed ~= 1 then
    ...
    LibStart=gg.getRangesList('libil2cpp.so')[2].start
--[[	// RVA: 0x3124C04 Offset: 0x3124C04 VA: 0x3124C04
...
DRAG[1]={}
DRAG[2]={}
DRAG[1].address=LibStart+0x3124C04 
DRAG[1].value='D2800020h'
DRAG[1].flags=4
DRAG[2].address=LibStart+(0x3124C04+0x4)
...
LibStart=gg.getRangesList('libil2cpp.so')[2].start
--[[
	// RVA: 0x31043D8 Offset: 0x31043D8 VA: 0x31043D8
	private void CheckCarForEnterPassenger(GameObject car, out bool toReturn) { }]]
DRAG=nil
DRAG={}
DRAG[1]={}
DRAG[2]={}
DRAG[1].address=LibStart+0x31043D8 
DRAG[1].value='D2800020h'
DRAG[1].flags=4
DRAG[2].address=LibStart+(0x31043D8+0x4)
DRAG[2].value='D65F03C0h'
DRAG[2].flags=4
gg.setValues(DRAG)
    gg.toast("༒ON༒")
...
gg.sleep(100)
LibStart=gg.getRangesList('libil2cpp.so')[2].start
--[[	// RVA: 0x30A51C0 Offset: 0x30A51C0 VA: 0x30A51C0
...
DRAG[1]={}
DRAG[2]={}
DRAG[1].address=LibStart+0x30A51C0 
DRAG[1].value='D2800020h'
DRAG[1].flags=4
DRAG[2].address=LibStart+(0x30A51C0+0x4)
DRAG[2].value='D65F03C0h'
DRAG[2].flags=4
gg.setValues(DRAG)
    gg.toast("༒ON༒")
...
LibStart=gg.getRangesList('libil2cpp.so')[2].start
--[[
// RVA: 0x37B7F80 Offset: 0x37B7F80 VA: 0x7778F5456C
...
DRAG[1]={}
DRAG[2]={}
DRAG[1].address=LibStart+0x37B7F80
DRAG[1].value='D2800020h'
DRAG[1].flags=4
DRAG[2].address=LibStart+(0x37B7F80+0x4)
DRAG[2].value='D65F03C0h'
DRAG[2].flags=4
gg.setValues(DRAG)
...
title =
    "╔क════════क⊱✫⊰क═══════क╗\n" ..
    "  CPM2 CLEAN v1.3.2.3 — ORGANIZED\n" ..
    "  Named offsets · No god packs\n" ..
...
-- ── Force Drift Mode ──────────────────────────────────────
-- RVA: 0x37AD8C0  CarController/input.SetDriftMode(bool)
...
-- RVA: 0x3653B30  PlayerNameChanged(string) — returns string
...
function longNameCPM2()
    gg.alert(
        ...
        "Method: PlayerNameChanged @ 0x3653B30"
    )
    local base = libBase()
...
--  UNLOCK / BYPASS CAR
-- ═══════════════════════════════════════════
local function _carLibBase()
    local r = gg.getRangesList("libil2cpp.so")
    if r and #r >= 2 then return r[2].start end
    return LibStart or lib
end

local function _carPatch(baseRva, patches)
    local base = _carLibBase()
    if not base then return false end
    local vals = {}
    for _, p in ipairs(patches) do
        vals[#vals+1] = {address = base + baseRva + p[1], flags = gg.TYPE_DWORD, value = p[2]}
    end
    gg.setValues(vals)
    return true
end
...
function unlockCars()
    local base = _carLibBase()
    if not base then gg.toast("❌ libil2cpp.so not found"); return end

    local DsOd = 0x3A0002C
    pcall(function()
        _carPatch(DsOd, {
            {0x618, 0xD503201F}, {0x6DC, 0xD503201F},
            {0x80C, 0xD503201F}, {0x858, 0xD503201F},
            {0x85C, 0x52800028},
        })
    end)

    local makecoin, get_Coin, get_Money = 0x3FF290C, 0x3FF422C, 0x3FF42D0
    local math2 = 1384120320 + (50 * 32)
    pcall(function()
        gg.setValues({
            {address = base + makecoin  + 0x18, flags = gg.TYPE_DWORD, value = 1384122227},
            {address = base + get_Coin  + 0x34, flags = gg.TYPE_DWORD, value = 0x52800000},
            {address = base + get_Money + 0x34, flags = gg.TYPE_DWORD, value = 0x52800000},
            {address = base + get_Money + 0x3C, flags = gg.TYPE_DWORD, value = math2},
        })
    end)

    local liveryV2Offset = 0x3FF46AC
    pcall(function()
        gg.setValues({
            {address = base + liveryV2Offset,     flags = gg.TYPE_DWORD, value = 0x2A1F03E0},
            {address = base + liveryV2Offset + 4, flags = gg.TYPE_DWORD, value = 0xD65F03C0},
        })
    end)

    pcall(function()
        gg.setValues({{address = base + 0x3E5B258, flags = gg.TYPE_DWORD, value = -698416192}})
    end)

    pcall(forceUnlockCarMethod)
    gg.toast("✅ Unlock Cars ON (Buy + Price + DB + UnlockCar)")
end

local _carIdOriginal = nil
local CAR_ID_OFFSET = 0x3A00BC8

local function movz_w2(val)
    if val < 0 or val > 500 then gg.toast("❌ ID 0-500"); return nil end
    return 0x52800000 | (val << 5) | 2
end

function setCarIdMenu()
    local base = _carLibBase()
    if not base then return end
    local addr = base + CAR_ID_OFFSET
    if not _carIdOriginal then
        local orig = gg.getValues({{address = addr, flags = gg.TYPE_DWORD}})
        if orig and orig[1] then _carIdOriginal = orig[1].value
        else gg.alert("❌ Cannot read Car ID offset"); return end
    end
    local choice = gg.choice({"🚗 Set Car ID", "♻️ Revert", "↩️ Back"}, nil,
        "Car ID (MOVZ W2)\n0x" .. string.format("%X", CAR_ID_OFFSET))
    if choice == 1 then
        local input = gg.prompt({"Car ID (0-500):"}, {""}, {"number"})
        if not input or not input[1] then return end
        local id = tonumber(input[1])
        if not id then return end
        local instr = movz_w2(id)
        if not instr then return end
        gg.setValues({{address = addr, flags = gg.TYPE_DWORD, value = instr}})
        gg.toast("✅ Car ID = " .. id)
    elseif choice == 2 and _carIdOriginal then
        gg.setValues({{address = addr, flags = gg.TYPE_DWORD, value = _carIdOriginal}})
        gg.toast("♻️ Reverted")
    end
end

function Menu_UnlockCar()
    local m = gg.choice({
        "🔓 Unlock Cars (Buy+Price+DB)",
        ...
        "📖 Help",
        "↩️ Back",
    }, nil, "🚗 UNLOCK / BYPASS CAR")
    if not m or m == 8 then return end
    if m == 1 then unlockCars()
    elseif m == 2 then forceUnlockCarMethod()
    elseif m == 3 then setCarIdMenu()
    elseif m == 4 then applyUnlockAll()
    elseif m == 5 then applyPolice()
    elseif m == 6 then xyzteleport()
    elseif m == 7 then copyCarRoomHelp()
    end
end
...
------------------------------------------------------------
-- IsEnoughCoins  @ 0x31DCD88  (already freePurchases)
-- IsEnoughMoney  @ 0x31DCE40
-- IsMoneyEnough  @ 0x31DCEF8
function bypassIsEnoughMoney()
    if _retTrue(0x31DCE40) then
        gg.toast("✅ IsEnoughMoney → true")
    end
end

function bypassIsMoneyEnough()
    if _retTrue(0x31DCEF8) then
...
function unlockAllPartsIsBought()
    if _retTrue(0x3679A48) then
        gg.toast("✅ Parts IsBought(body,type,item) → true")
    end
end

-- Air suspension system IsBought(CarInfo) @ 0x3726688
function unlockAirSusIsBought()
    if _retTrue(0x3726688) then
        gg.toast("✅ AirSuspension IsBought → true")
    end
end

-- Generic car inventory IsBought(CarInfo) @ 0x3726688 already
-- Police @ 0x37E3B7C already in script

------------------------------------------------------------
-- CAR BAN / TRADE / NFS FILTER
-- IsBannedCar              @ 0x337395C → false
-- CheckerNFSCars           @ 0x3102558 → RET (skip)
-- CheckTradeCarNotAllowed  @ 0x33739E4 / 0x33739E4 → zero
------------------------------------------------------------
function bypassBannedCars()
    if _retFalse(0x337395C) then
...
-- IsVehicleAllowed(EventType, VehicleClass) @ 0x35D4640 → true
------------------------------------------------------------
function bypassEventVehicleRestriction()
    if _retTrue(0x35D4640) then
...
-- AddPenaltyDistance @ 0x3002358
-- AddPenaltyTime     @ 0x3002368
------------------------------------------------------------
...
-- CheckerEngineCheating @ 0x3854158
------------------------------------------------------------
function bypassEngineCheatDetect()
    if _retVoid(0x3854158) or _retZero(0x3854158) then
...
-- get_CheckHashLimitExceeded @ 0x3945444 → false
-- get_CorrectHash            @ 0x3945454 → true (AdvancedBoolean layout may differ)
------------------------------------------------------------
function bypassHashLimit()
    if _retFalse(0x3945444) then
...
-- UtilsVehicles.IsBlackVehicle @ 0x3649BB4 → false
------------------------------------------------------------
function bypassBlackVehicleCheck()
    if _retFalse(0x3649BB4) then
...
------------------------------------------------------------
-- RACE CHEAT FLAGS
-- IsCheatFinish @ 0x3002E5C → false (already racing path)
------------------------------------------------------------
function bypassIsCheatFinish()
    if _retFalse(0x3002E5C) then
...
------------------------------------------------------------
-- ⚡ ONE-CLICK EVERYTHING FREE / BYPASS
-- Uses every safe RVA from the 1.3.2.3 dump table
------------------------------------------------------------
function OneClickEverythingBypass()
    gg.toast("⚡ Applying FULL FREE / BYPASS pack…")
    gg.sleep(150)

    -- MONEY / CURRENCY
    pcall(applyMaxMoney)
    pcall(freePurchases)
    pcall(bypassIsEnoughMoney)
    pcall(bypassIsMoneyEnough)
    pcall(bypassAllCurrencyChecks)
    pcall(freeBodykitPrices)
    pcall(freeFuelConsumption)
    ...
    pcall(unlockAirSusIsBought)
    pcall(unlockCars)
    pcall(forceUnlockCarMethod)

    -- GEARBOX / ENGINE
    pcall(unlockallgb)
    pcall(unlcokpartenmgine)
    ...
    -- ANTI-CHEAT / SERVER
    pcall(bypassIsCheatActivated)
    pcall(bypassPassengerKickPack)
    pcall(bypassMainCarCondition)
    pcall(bypassBlackVehicleCheck)
    pcall(bypassIsCheatFinish)
    pcall(bypassServerIsCheatFinish)
    pcall(bypassEngineCheatDetect)
    pcall(bypassHashLimit)
    pcall(forceCorrectHash)
    pcall(bypassApexCarBreak)
    pcall(bypassserver)
    pcall(forceCarDebugTools)

    -- RACE / RALLY
    pcall(dumpRaceWinPack)
    pcall(dumpRallyWinPack)
    pcall(forceDumbAI)
    pcall(bypassCircuitCheckParent)
    pcall(bypassAddPenaltyWriters)

    -- BANNED / TRADE / EVENT
    pcall(unlockBannedTradePack)
    pcall(bypassEventVehicleRestriction)
    pcall(bypassNFSCarCheck)
    pcall(bypassTradeCarNotAllowed)

    pcall(applyAllExtraBypasses)

    gg.alert(
        "✅ ONE-CLICK EVERYTHING FREE / BYPASS DONE\n\n" ..
        "Applied from dump 1.3.2.3:\n" ..
        "• Money + Coins + Free Purchases\n" ..
        "• All shop / parts / paints / bodykits free\n" ..
        "• Unlock Cars + Police + Houses +Slots\n" ..
        "• Gearbox + Engine fully open\n" ..
        "• Anti-Cheat + Anti-Kick + Server checks\n" ..
        "• Race / Rally / Drag helpers\n" ..
        "• Banned / Trade / Event vehicles\n\n" ..
        "Use in Lobby / Garage first, then enter room."
    )
end
...
-- Free bodykits: GetCoinPriceForKit @ 0x372A524 → 0
-- get_BodyKitsMoneyPrice @ 0x361DBE8 → 0
...
-- CircuitRaceControlller.CheckParent @ 0x3165F18
function bypassCircuitCheckParent()
    if _retTrue(0x3165F18) then
        gg.toast("✅ CircuitRace CheckParent → true")
    end
end

-- CircuitCarStearing.ChangeSteeringState @ 0x3158ADC → dumb AI
function forceDumbAI()
    if _retVoid(0x3158ADC) then
...
function dumpRaceWinPack()
    _retTrue(0x30035F0)   -- IsFinalLap always true
    _retFalse(0x3002E5C)  -- IsCheatFinish false
    _retVoid(0x3002358)   -- AddPenaltyDistance
    _retVoid(0x3002368)   -- AddPenaltyTime
...
function dumpRallyWinPack()
    _retFalse(0x35545B4)  -- not off-road
    _retZero(0x3553538)   -- penalty 0
    _retZero(0x3553540)   -- missed 0
    _retVoid(0x3553B8C)   -- AddPenalty RET
    _retVoid(0x3555898)   -- ApplyMissed RET
...
function dumpSlotLimitBypass()
    pcall(slotmod)
    gg.toast("✅ Slot limit gate (0x367D090+0xC0) via slotmod")
end
...
function noDamageOFF()
    gg.clearList()
    pcall(function()
        local b2 = (LibStart or _libBaseSafe()) + 0x39AE43C
...
function instantServiceZero()
    -- get_CurrentUnixTimeSeconds @ 0x360E718 → force large / bypass wait
    pcall(bypasseservvicetrime)
    pcall(bypassgtime)
...
function HomeMenu()
    ...
    elseif choice == 2 then
        -- Individual unlocks only (no Unlock All mega pack)
        local u = gg.multiChoice({
            ...
            "🛞 Wheels",
            "📦 32 slots / SlotMod",
        }, nil, "🔓 UNLOCKS (individual)")
        if u then
            if u[1] then pcall(applyPolice); pcall(policeautoset) end
            if u[2] then pcall(applyAirSus); pcall(unlockAirSuspension) end
            if u[3] then pcall(hasHouse) end
            if u[4] then pcall(paint) end
            if u[5] then pcall(bodykit) end
            if u[6] then pcall(unlockClothes) end
            if u[7] then pcall(unlockFlags) end
            if u[8] then pcall(wheelUnlock) end
            if u[9] then
                local s = gg.choice({"ThirtyTwoSlots", "slotmod", "Back"}, nil, "SLOTS")
                if s == 1 then pcall(ThirtyTwoSlots) elseif s == 2 then pcall(slotmod) end
            end
        end
    ...
    elseif choice == 4 then
        pcall(menubypassgb)
        if not menubypassgb then pcall(Menu_bypassmenu) end
    ...
    elseif choice == 5 then
        ...
        if r == 1 then pcall(hook1)
        elseif r == 2 then
            -- toggle lemans
            if lemans == on then pcall(lemans1); lemans = off
            else pcall(lemans2); lemans = on end
        elseif r == 3 then pcall(hook3)
        elseif r == 4 then pcall(hook4)
        elseif r == 5 then pcall(hook5)
        elseif r == 6 then pcall(hook6)
        elseif r == 7 then pcall(activeall1)
        end
    ...
    elseif choice == 6 then
        local b = gg.choice({
            ...
            "⏱️ Service Time Bypass",
            "🕵️ Engine Cheat Detect off",
            "🔐 Hash Limit Bypass",
            "✔️ Force Correct Hash",
            "🖤 Black Vehicle off",
            "↩️ Back",
        }, nil, "🛡️ SERVER / BYPASS")
        if b == 1 then pcall(bypassserver)
        elseif b == 2 then pcall(bypassenginecomp)
        elseif b == 3 then pcall(bypasseservvicetrime)
        elseif b == 4 then pcall(bypassgdetece)
        elseif b == 5 then pcall(bypassHashLimit)
        elseif b == 6 then pcall(forceCorrectHash)
        elseif b == 7 then pcall(bypassBlackVehicleCheck)
        end
    ...
    elseif choice == 9 then
        -- Teleport & ESP combined
        if Menu_LocationTeleport then
            local tm = gg.choice({
                "📍 Location Teleport",
                "👁️ Player ESP / Names / Positions",
                "📍 XYZ Teleport",
                "🔐 Room Password",
                "↩️ Back",
            }, nil, "📍 TELEPORT & ESP")
            if tm == 1 then pcall(Menu_LocationTeleport)
            elseif tm == 2 then pcall(Menu_PlayerESP)
            elseif tm == 3 then pcall(xyzteleport)
            elseif tm == 4 then pcall(checkpassword)
            end
        else
            pcall(xyzteleport)
        end
    ...
    elseif choice == 11 then
        if Menu_PhysicsGod then pcall(Menu_PhysicsGod)
        else
            local p = gg.choice({"Infinite Nitro try","Infinite Fuel try","No Damage (Tyres 100%)","Clear freezes","Back"}, nil, "PHYSICS")
            if p == 1 then pcall(infiniteNitroON)
            elseif p == 2 then pcall(infiniteFuelON)
            elseif p == 3 then pcall(applyTyres100)
            elseif p == 4 then gg.clearList(); gg.clearResults(); gg.toast("Cleared")
            end
        end

    elseif choice == 12 then
        -- Fun physics from prank + community
        local f = gg.choice({
            "Dance Car 1", "Dance Car 2", "Dance Car 3",
            "Hover Car", "Fly Cars All", "Shadow Wall", "Towing",
            "Wall Hack", "EMP Invisible", "Back",
        }, nil, "🎉 FUN PHYSICS")
        if f == 1 then pcall(danceCar1)
        elseif f == 2 then pcall(danceCar2)
        elseif f == 3 then pcall(danceCar3)
        elseif f == 4 then pcall(hovercar)
        elseif f == 5 then pcall(flycarall)
        elseif f == 6 then pcall(shadowwall)
        elseif f == 7 then pcall(towingthecars)
        elseif f == 8 then pcall(wallHack)
        elseif f == 9 then pcall(togglePatch)
        end
    ...
    elseif choice == 14 then
        pcall(Menu_Achievement)

    elseif choice == 15 then
        if Menu_CommunityExtras then pcall(Menu_CommunityExtras)
        elseif Menu_CPM1Style then pcall(Menu_CPM1Style)
        else
            local e = gg.choice({"Remove Bumper","ID Changer","Fast Character","Back"}, nil, "EXTRAS")
            if e == 1 then pcall(RB2)
            elseif e == 2 then pcall(IDChanger)
            elseif e == 3 then pcall(fastCharacter)
            end
        end

    elseif choice == 16 then
        -- Useful dump bypasses only (NOT full one-click everything)
        if Menu_ExtraUnlockBypass then
            pcall(Menu_ExtraUnlockBypass)
        else
            local x = gg.choice({
                "Currency checks open",
                "Parts IsBought",
                "Banned/Trade bypass",
                "Race penalty writers RET",
                "Back",
            }, nil, "EXTRA BYPASSES")
            if x == 1 then pcall(bypassAllCurrencyChecks)
            elseif x == 2 then pcall(unlockAllPartsIsBought)
            elseif x == 3 then pcall(unlockBannedTradePack)
            elseif x == 4 then pcall(bypassAddPenaltyWriters)
            end
        end

    elseif choice == 17 then
        gg.clearResults(); gg.clearList(); gg.toast("Freezes cleared")

    elseif choice == 18 then
        gg.clearResults(); gg.clearList()
        gg.toast("Bye — CPM2 Clean")
        running = false
        os.exit()
    end
end
