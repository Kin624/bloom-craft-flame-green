--[[
  *** CPM2 CLEAN — OFFSETS UPDATED TO LATEST DUMP ***
  ...
  Offset table + hard-coded RVAs updated from latest dump.cs (Aug 2026).

  Update offsets only in the OFFSET TABLE header when game updates.
--]]

--[[ ══════════════════════════════════════════════════════════
  CPM2  — OFFSETS UPDATED TO LATEST DUMP
  Sources merged:
    • CPM2 opensource (Kinzi – unlocks, prank, boosters, achievements)
    • #Cpm2CoinsHook.lua (Kirito – race/drag/rally + getField)
    • dump.cs LATEST (47MB) – RVAs remapped Aug 2026

  Target : Car Parking Multiplayer 2  (Unity IL2CPP, Android)
  Library: libil2cpp.so

  ═══════════════════════════════════════════════════════════
  OFFSET TABLE — libil2cpp.so  (RVA = Offset from latest dump)
  ═══════════════════════════════════════════════════════════

  MONEY / PLAYERPREFS
    0x67521D0  PlayerPrefs.GetFloat(string,float)
    0x6752138  PlayerPrefs.SetFloat
    0x675225C  PlayerPrefs.SetString

  CURRENCY (CurrencyManagerController)
    0x31DC7BC  SetMoneyPrefs
    0x31DC858  SetMoney
    0x31DCAEC  SetCoins
    0x31DCAF4  AddMoney
    0x31DCD88  IsEnoughCoins → force true
    0x31DCE40  IsEnoughMoney
    0x31DCEF8  IsMoneyEnough

  RACE / LAP (LapRaceCar)
    0x30035F0  IsFinalLap → always true
    0x3002E5C  IsCheatFinish → false
    0x30031AC  OnLapFinish
    0x3003424  GetBestLap
    0x3003998  CalculatePos
    0x3002358  AddPenaltyDistance → RET
    0x3002368  AddPenaltyTime → RET
    0x3003088  GetTotalProgress
    0x3003168  GetPenaltyDistance
    0x2FFA898  GetPenaltyTime
    0x30037C8  GetBestLapTime
    0x30033DC  GetCurrentLapTime
    0x2FFA8A4  GetTotalTime
    0x2FFA878  GetClearTime
    0x3165F18  CircuitRaceControlller.CheckParent
    0x3158ADC  CircuitCarStearing.ChangeSteeringState
    0x319D114  LemanRaceController.SetTime

  RALLY (RallyCar)
    0x35545B4  IsCarOffRoad
    0x3553538  get_Penalty
    0x3553540  get_MissedCheckpoints
    0x3553B8C  AddPenalty
    0x3555898  ApplyMissedCheckpointsPenalty

  GEARBOX / ENGINE
    0x3884C1C  CheckGearboxNotCompatible
    0x3603ED4  CheckNotCompatibleEngine
    0x360E7F8  get_CurrentUnixTimeSeconds
    0x3854158  CheckerEngineCheating
    0x39AE43C  get_TyreHealth

  UNLOCK / SHOP / RANK
    0x30C5730  hasHouse
    0x3673D24  CheckPaintBought
    0x372A524  GetCoinPriceForKit
    0x361DBE8  get_BodyKitsMoneyPrice
    0x32F6FE4  GetClassVehicle
    0x3726688  AirSus IsBought
    0x3069CC0  GetOwnRank
    0x309EE0C  UnlockCar
    0x3679A70  Parts IsBought

  ANTI-CHEAT / HASH
    0x3945444  get_CheckHashLimitExceeded → false
    0x3945454  get_CorrectHash → true
    0x337395C  IsBannedCar → false
    0x33739E4 / 0x3373AAC  CheckTradeCarNotAllowed → 0
    0x35D4640  IsVehicleAllowed → true
    0x3649BB4  IsBlackVehicle → false

  NOTE: A few secondary anti-kick / room / debug offsets may still
  need live verification. Test race win + free buy first.
...
local supportedVersions = {"1.3.2.3", "1.3.3.0", "1.3.3.1", "1.4.0.0"}  -- expand after testing real versionName
...
local UNLOCK_ALL_OFFSETS = {
    0x3673D48, 0x3673D24, 0x3673D24, 0x313BAA0,
    0x2ED8F40, 0x3679A70, 0x33A5348
}
...
function bypasseservvicetrime()  -- get_CurrentUnixTimeSeconds @ 0x360E7F8
    libs("libil2cpp.so")
    gg.setValues({
        {address=lib+0x360E7F8,   flags=gg.TYPE_DWORD, value="h200080D2"},
        {address=lib+0x360E7F8+4, flags=gg.TYPE_DWORD, value="hC0035FD6"},
    })
...
function bypassgtime()   -- alias / bypass GB Time
    libs("libil2cpp.so")
    gg.setValues({
        {address=lib+0x360E7F8+4, flags=4, value="h200080D2"},
        {address=lib+0x360E7F8,   flags=4, value="hC0035FD6"},
    })
...
local Time1  = 0x3003424  -- LapRaceCar.GetBestLap;  local Time2  = 0x30037C8
local Time3  = 0x30033DC;  local Time4  = 0x300342C
local Time6  = 0x3003798;  local Time7  = 0x2FFA878
local Time8  = 0x2FFA8A4;  local Time9  = 0x3A7A8B0
...
local Distance1 = 0x3003088;  local Distance2 = 0x3003168
local Distance3 = 0x3003088;  local Distance4 = 0x2FFF9A8
...
gg.sleep(100)
    ...
    gg.setValues(DRAG)
        local DRAG = {
        {address = base + 0x3673D48, value = '~A8 MOV  X0, #0x1', flags = 4},
        {address = base + 0x3673D48 + 4, value = '~A8 RET', flags = 4}
    }
    gg.setValues(DRAG)
         local KINZI = {
    ...
    gg.setValues(KINZI)
    local KINZI = {
        {address = LibStart + 0x3673D48, value = "-2999674700105252832", flags = gg.TYPE_QWORD},
    }
    gg.setValues(KINZI)
    gg.toast(" unlocked")
    gg.toast("༒ON༒")
...
-- CheckTradeCarNotAllowed  @ 0x33739E4 / 0x3373AAC → zero
------------------------------------------------------------
...
function instantServiceZero()
    -- get_CurrentUnixTimeSeconds @ 0x360E7F8 → force large / bypass wait
    pcall(bypasseservvicetrime)
    pcall(bypassgtime)
